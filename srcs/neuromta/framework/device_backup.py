...
class _InternalDeviceCoreSignal:
    ...
    UPDATE_TIME_COMPANION_ONLY_REQ = 1  # DEVICE -> CORE >> request to update the companion core and return its remaining cycles
    CORE_SYNC_REQ    = 2  # DEVICE -> CORE >> terminate the current process
    TERMINATE        = 3
    
    UPDATE_TIME_REQ  = 4  # CORE -> DEVICE >> alert that the device the current remaining time and request for the timestamp update
    MAIN_IDLE        = 6  # CORE -> DEVICE >> alert that the current main process has been finished
    EXCEPTION        = 7  # CORE -> DEVICE >> alert that the exception occurred for the given core
    CORE_SYNC_RSP    = 8  # CORE -> DEVICE >> alert that the core has finished its execution and is ready for the core to be synchronized (send core states as a payload)
